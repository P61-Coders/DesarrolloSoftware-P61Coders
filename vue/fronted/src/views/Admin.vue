  ...
  <v-app>
    <v-card class="overflow-hidden">
      ...
      >
        ...
        <v-btn class="ma-2" color="grey lighten-1" dark @click="salir()">
          <v-icon dark left> {{ icon }} </v-icon>Logout
        </v-btn>

        <template v-slot:extension>
          <v-tabs align-with-title>
            <v-tab :to="{ name: 'GestorUsuarios' }" v-if="isAdmin()"
              >Usuarios</v-tab
            >
